...
public class FunctionLevelPairs
{
    ...
    public static class TokenizerMapper extends Mapper<Object, Text, mypair, IntWritable>
    {
        ...
        public void map(Object key,Text value,Context context)throws IOException, InterruptedException
        {
            ...
            int distance = 4;// Can be changed to 2,3,4 
             Comparator<String[]> arrayComparator = (arr1, arr2) -> {
            return Arrays.compare(arr1, arr2);
            };
            Map<String[],Integer> associative_array = new TreeMap<>(arrayComparator);
            Text word1=new Text();
            Text word2=new Text();
                ...
                if(words.contains(tokens[i]))
                {
                    for(int j=i;j<=Math.min(i+distance,tokens.length-1);j++)
                    {
                        if(words.contains(tokens[j]))
                        {
                            ...
                            String[] temp = {tokens[i],tokens[j]};
                            associative_array.put(temp,associative_array.getOrDefault(temp, 0)+1);
                        }
                    }
                }
            }
            for(Map.Entry<String[],Integer> entry:associative_array.entrySet())
            {
                val.set(entry.getValue());
                word1.set(entry.getKey()[0]);
                word2.set(entry.getKey()[1]);
                mypair p = new mypair(word1,word2);
                context.write(p,val);
            }
            // associative_array.
        }
