...
public class ClassLevelStripes {
    ...
    public static class TokenizerMapper extends Mapper<Object, Text, Text, MapWritable>
    {
        ...
        Map<String,MapWritable> associative_array = new TreeMap<>();
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException
        {
            ...
            for(int i = 0; i < tokens.length; i++)
            {
                ...
                {
                    if(!associative_array.containsKey(tokens[i]))
                    {
                        associative_array.put(tokens[i],new MapWritable());
                    }
                    MapWritable h = associative_array.get(tokens[i]);
                    for(int j = i; j <= Math.min(i + distance, tokens.length - 1); j++)
                    {
                        ...
                        if(words.contains(tokens[j]))
                        {
                            ...
                            IntWritable cnt = (IntWritable) h.get(token);
                            if(cnt!=null)
                            {
                    ...
                    associative_array.put(tokens[i],h);
                }
            }
        ...
        {
            for(Map.Entry<String,MapWritable> entry:associative_array.entrySet())
            {
                MapWritable temp = entry.getValue();
                Text word1 = new Text(entry.getKey());
                context.write(word1,temp);
            }
        }
    }
